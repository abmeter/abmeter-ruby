...
describe 'Assignment Algorithm Fairness' do
  ...
  describe 'Space allocation fairness' do
    ...
    it 'ensures mutual exclusion within a space' do
      ...
      sample_size.times do |i|
        ...
        # resolved_value is the exact variant value the user was assigned (a
        # stored constant, not a computed float), so == is reliable here.
        in_price_experiment = price_exposure[:resolved_value] == 0.2 # rubocop:disable Lint/FloatComparison
  ...
  describe 'Control group independence' do
    ...
    it 'ensures control groups have similar user quality across experiments' do
      ...
      RSpec.configuration.reporter.message("Color experiment control avg: #{color_control_avg}")
      RSpec.configuration.reporter.message("Price experiment control avg: #{price_control_avg}")
      RSpec.configuration.reporter.message("Difference: #{(color_control_avg - price_control_avg).abs}")
    ...
    it 'demonstrates that control group differences decrease with larger sample sizes' do
      ...
      sample_sizes = [100, 500, 1000, 5000, 10_000]
      differences = []
      ...
      sample_sizes.each do |sample_size|
        ...
        next unless color_control_count > 0 && price_control_count > 0
        ...
        RSpec.configuration.reporter.message("Sample size #{sample_size}: Color control #{(color_rate * 100).round(1)}%, " \
